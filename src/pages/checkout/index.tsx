...
const Checkout = () => {
  ...
  const { data: priceData } = trpc.useQuery(['products.get-total-price', { paymentMethod, productIds: localStorage.getItem('productIds')! }], {
    enabled: (paymentMethod === 'qr-sol' || paymentMethod === 'qr-usd') && (!!localStorage.getItem('productIds'))
  });
  const [urlParams, setUrlParams] = useState<TransferRequestURLFields>();

  console.log({ isData: (paymentMethod === 'qr-sol' || paymentMethod === 'qr-usd') && (!!localStorage.getItem('productIds')) });
        ...
        <section className="flex-col w-full px-6 py-10 lg:p-20 bg-slate-100">
          ...
          {(paymentMethod === 'qr-sol' || paymentMethod === 'qr-usd') &&
            <section className='py-6 border-4 rounded-sm border-primary'>
              ...
              <h2 className="text-5xl font-bold tracking-widest text-center font-heading text-primary">Scan this and pay</h2>
            </section>
          }
          <h2 className="mt-8 mb-4 text-2xl sm:text-3xl lg:text-4xl">
            Order Summary
